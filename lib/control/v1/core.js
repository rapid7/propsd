...
function Core(app, sources) {
  ...
  routes.status.get((req, res) => {
    ...
    const v1sources = health.sources.map((source) => {
      return Object.assign(source, {status: source.ok ? 'okay' : 'fail'});
    });

    const v1indices = sources.indices.map((index) => {
      const indexStatus = index.status();

      return Object.assign(indexStatus, {running: indexStatus.state !== Source.CREATED});
    });
