...
const StringTemplate = require('../util/string-template');
...
class Sources extends Parser {
  ...
  updateSources(update) {
    ...
    update.forEach(function (config) {
      ...
      // Parse configuration parameters as TemplateStrings
      const parameters = config.parameters || {};
      Object.keys(parameters).forEach((key) => {
        parameters[key] = StringTemplate.coerce(parameters[key], _this.metadata.properties);
      });
