...
class PluginManager extends EventEmitter {
  ...
  constructor(storage, options) {
    ...
    this.index.on('update', (logMetadata) => {
      Log.log('INFO', `Updated source ${this.index.name}`, Object.assign({},
          this._defaultLogMetadata, {
            sourceName: this.index.name,
            sourceType: this.index.type
          },
          logMetadata));
    ...
    this.metadata.on('error', (error) => {
      this._error(error, this.metadata);
    ...
    this._defaultLogMetadata = {
      instance: Config.get('properties') || {}
    };
  }
   ...
   */
  initialize() {
    Log.log('INFO', 'Initializing index and metadata', this._defaultLogMetadata);
    this._running = true;
   ...
   */
  shutdown() {
    Log.log('INFO', 'Shutting down sources', {
  ...
  _registerSourceEvents(source) {
    source.on('init', (logMetadata) => {
      Log.log('INFO', `Initializing ${source.type} source ${source.name}`,
        Object.assign({
    ...
    source.on('update', (logMetadata) => {
      const method = (source.name === 'consul') ? 'SILLY' : 'INFO';

      Log.log(method, `Updated source ${source.name}`,
        Object.assign({
          sourceName: source.name,
          sourceType: source.type
        }, logMetadata));
    ...
    source.on('no-update', (logMetadata) => {
      Log.log('DEBUG', `${source.name} has no update to its underlying source data.`,
        Object.assign({
          sourceName: source.name,
          sourceType: source.type
        }, logMetadata));
    ...
    source.on('stop', (logMetadata) => {
      Log.log('INFO', `Stopping ${this.type} source ${this.name} polling`,
        Object.assign({
          sourceName: source.name,
          sourceType: source.type
        }, logMetadata));
    ...
    source.on('shutdown', (logMetadata) => {
      Log.log('INFO', `Shutting down ${source.type} source ${source.name}`,
        Object.assign({
          sourceName: source.name,
          sourceType: source.type
        }, logMetadata));
