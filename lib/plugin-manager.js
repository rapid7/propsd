...
class PluginManager extends EventEmitter {
  ...
  constructor(storage, options) {
    ...
    this.index.on('update', (logMetadata) => {
      Log.INFO(`Updated source ${this.index.name}`, Object.assign({},
  ...
  _registerSourceEvents(source) {
    ...
    source.on('startup', (logMetadata) => {
      Log.INFO(`Starting ${source.type} source ${source.name}`, Object.assign({
        sourceName: source.name,
        sourceType: source.type
      }, logMetadata));
    });
    ...
    source.on('update', (logMetadata) => {
      ...
      Log[method](`Updated source ${source.name}`,
        Object.assign({
          sourceName: source.name,
          sourceType: source.type
        }, logMetadata));
