...
class PluginManager extends EventEmitter {
  ...
  _registerSourceEvents(source) {
    source.on('init', (logMetadata) => {
      Log.info(`Initializing ${source.type} source ${source.name}`, Object.assign(this._defaultLogMetadata, {
    ...
    source.on('startup', (logMetadata) => {
      Log.info(`${source.name} started up successfully.`, Object.assign(this._defaultLogMetadata, {
        sourceName: source.name,
        sourceType: source.type
      }, logMetadata));
    });
    ...
    source.on('shutdown', (logMetadata) => {
      Log.info(`Shutting down ${source.type} source ${source.name}`, Object.assign(this._defaultLogMetadata, {
        sourceName: source.name,
        sourceType: source.type
      }, logMetadata));
    });
    ...
    source.on('update', (instance, logMetadata) => {
      Log.info(`Updated source ${instance.name}`, Object.assign(this._defaultLogMetadata, {
        sourceName: source.name,
        sourceType: source.type
      }, logMetadata));
      this.storage.update();
    ...
    source.on('no-update', (logMetadata) => {
      Log.debug(`${source.name} has no update to its underlying source data.`, Object.assign(this._defaultLogMetadata, {
        sourceName: source.name,
        sourceType: source.type
      }, logMetadata));
    });
