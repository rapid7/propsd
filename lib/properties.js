...
class Properties extends EventEmitter {
   ...
   */
  dynamic(source, namespace, options) {
    this.layers.push(new Layer.Dynamic(source, namespace, options));
  }
   ...
   * @param {String}  options
   */
  static(properties, namespace, options) {
    this.layers.push(new Layer.Static(properties, namespace, options));
  }
  ...
  build() {
    ...
    this._building = setTimeout(() => {
      ...
      // Merge non-rendered layers into their own object. This can be consumed
      // as an input by template renderers.
      const persistent = {};
        ...
        .forEach((layer) => {
          if (!layer.namespace) {
            if (layer.render) {
              ...
              Object.assign(persistent, layer.properties);
            }
            return;
          }
