...
class Properties extends EventEmitter {
  ...
  get sources() {
    return Immutable.List()
      ...
      .concat(this._layers.map((layer) => layer.source).filter((source) => !!source))
  ...
  /**
   * Getter for properties to cast it to an Object
   *
   * @return {Object}
   */
  get properties() {
    return this._properties.toJS();
  }

  /**
   * Getting for layers to cast it to an Array
   *
   * @return {Array}
   */
  get layers() {
    return this._layers.toJS();
  }
  ...
  dynamic(source, namespace) {
    this._layers = this._layers.push(new Layer.Dynamic(source, namespace));
  }
  ...
  static(properties, namespace) {
    this._layers = this._layers.push(new Layer.Static(properties, namespace));
  }
  ...
  initialize() {
    ...
    return Promise.all(
      this._layers.map((layer) => layer.initialize())
    ).then((layers) => {
  ...
  build() {
    ...
    this._building = setTimeout(() => {
      ...
      this._properties = Immutable.Map(properties);
