  ...
  constructor(configs, properties) {
    ...
    configs.forEach((config) => {
      if (!config.hasOwnProperty('type') || !config.type) {
        Log.log('WRAN', 'Source configuration does not have a `type` parameter! Ignoring.');
        return;
      }
      ...
      if (!config.hasOwnProperty('name') || !config.name) {
        ...
        Log.log('WRAN', `Source configuration does not have a \`name\` parameter! Generated ${config.name}`);
      }
