 ...
 * @return {Object}             Returns reference to the destination Object
 */
function mergeDeep(source, destination) {
  destination = destination || {};
  if (!(source instanceof Object)) return destination;

  // Concatenate Arrays
  if (source instanceof Array && destination instanceof Array) return destination.concat(source);

  Object.keys(source).forEach(function (key) {
    // Don't deep-merge null or functions
    if (source[key] !== null && !(source[key] instanceof Function) && source[key] instanceof Object) {
      // Ensure that destination is the same type as source
      if (!destination.hasOwnProperty(key) || destination[key].constructor !== source[key].constructor) {
        destination[key] = new (source[key].constructor)();
      }

      return (destination[key] = mergeDeep(source[key], destination[key]));
    }
...
exports.merge.deep = mergeDeep;
exports.clone.deep = mergeDeep;
...
function equal(a, b) {
  ...
  if (a instanceof Array && b instanceof Array) {
    // Equal Arrays must be the same length
